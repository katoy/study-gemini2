...
- **全体:** 96%（GUI テストはスキップ）
- **GUI 以外:** 100%（CI で強制）
- **対象ファイル:** agents/, board.py, game.py, config/, utils/ など

### CI/CD パイプライン

GitHub Actions で以下の CI チェックが自動実行されます：

- **Lint チェック:** ruff による Python コード品質検査
- **型チェック:** mypy による型安全性検証
- **テスト実行:** pytest による 200+ のユニットテスト
- **カバレッジ確認:** GUI 除外時の 100% カバレッジ確認

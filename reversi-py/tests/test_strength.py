...
Negamax、Transposition、Pattern、AlphaZero エージェントが初期化でき、
基本的な着手を返すことを確認。複数エージェントは GainAgent に勝てることを確認。
...
class TestNegamaxBeatsGainAgent:
    ...
    board_size=8 の理由:
      - 実際のゲーム盤面（8x8）での強さを検証
      - time_limit_ms=50 で探索時間を制限
      - 1 ゲーム最大 3 秒（60 手 × 50ms）
      - 2 ゲーム合計で約 6 秒以内
    """
...
class TestAlphaZeroAgent:
    ...
    def test_alpha_zero_initializes(self) -> None:
        """AlphaZeroAgent が正常に初期化される。"""
        agent = AlphaZeroAgent(n_simulations=self.N_SIMULATIONS)
        assert agent is not None, "AlphaZeroAgent の初期化に失敗"

    def test_alpha_zero_returns_move(self) -> None:
        """AlphaZeroAgent が 8x8 初期盤面で合法手を返す。"""
        agent = AlphaZeroAgent(n_simulations=self.N_SIMULATIONS)
